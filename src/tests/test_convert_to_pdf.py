...
class BaseTestConvertToPDF:
    ...
    def _convert_and_verify(self, filename, expected_strings=None):
        ...
        # Verify PDF content with  given expected strings
...
class TestSuccessfulConversions(BaseTestConvertToPDF):
    def test_heartbeat(self):
        response = self.client.get("/heartbeat")
        assert response.status_code == 200, "libreoffice server is not running"
    ...
    def test_convert_valid_pptx(self):
        self._convert_and_verify("Presentation.pptx", ["Presentation", "Test"])

    def test_convert_valid_txt(self):
        self._convert_and_verify("simple.txt", ["this is a test"])
...
class TestFileUploadEdgeCases(BaseTestConvertToPDF):
...
class TestErrorHandling(BaseTestConvertToPDF):
    ...
    def test_corrupt_docx(self):
        ...
        # Confirm that valid docx still works afterward
